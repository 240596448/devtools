Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("step step-by-step", Ложь, "ожидать продолжения после каждлого шага (останавливать выполнение до подтверждения)")
			.Флаг();
...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	ИспользоватьПошаговоеВыполенние  = Команда.ЗначениеОпции("step");
	
	ЗамерНачало = ТекущаяДата();
	Лог.Информация("Начало: %1", ЗамерНачало);

	МенеджерСинхронизацииСХранилищем = Новый МенеджерСинхронизацииСХранилищем(ИспользоватьПошаговоеВыполенние);
