...
Процедура ВыгрузитьОбъектыКонфигурациВФайлы(КаталогИсходников, ИмяФайлаСпискаОбъектов) Экспорт
	...
	Параметры.Добавить(СтрШаблон("/DumpConfigToFiles %1", Служебный.ОбернутьВКавычки(КаталогИсходников)));
	Параметры.Добавить(СтрШаблон("-format %1", "Hierarchical"));

	Параметры.Добавить(СтрШаблон("-listFile %1", Служебный.ОбернутьВКавычки(ИмяФайлаСпискаОбъектов)));
...
Процедура ЗахватитьОбъектыВХранилище(СписокОбъектовХранилища) Экспорт
	...
	ИмяФайлаСпискаОбъектовХранилища = Служебный.ЗаписатьВоВременныйФайл(Текст, "objects");
