...
Функция СформироватьСписок(мОбъекты) Экспорт
	...
	Для каждого Объект Из мОбъекты Цикл
		...
		Пока Истина Цикл
			...
			Лог.Информация("Ищем %1 в каталоге %2", ОбъектПоиска, КаталогПоиска);
...
Функция ВыполнитьГитКоманду(КоманднаяСтрока, ДопустимыйКодВозврата = 0) Экспорт
	...
	Если РежимВыводаСообщений Тогда
		Лог.Информация("message from git: " + Символы.ПС + ВыводКоманды);
	КонецЕсли;
...
Функция ПолучитьВыводКоманды() Экспорт
	...
	мВыводКоманды = Новый Массив();
	Для каждого Стр Из СтрРазделить(ВыводКоманды, Символы.ПС, Ложь) Цикл
		Если ПустаяСтрока(Стр) Тогда
			Продолжить;
		КонецЕсли;
		мВыводКоманды.Добавить(СокрП(Стр));
	КонецЦикла;

	Возврат СтрСоединить(мВыводКоманды, Символы.ПС);
