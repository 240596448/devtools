...
Процедура ОтменаВыполнения() Экспорт
	...
	Лог.Информация("Отмена захвата в хранилище");
	УправлениеКонфигуратором.ОтменитьЗахватОбъектовВХранилище(Хранилище.Путь, Хранилище.Пользователь, Хранилище.Пароль, , Истина);
...
Процедура ВыполнитьСинхронизациюИзВеткиРазработкиВВеткуХранилищем() Экспорт
	...
	Репозиторий.ВыполнитьГитКоманду("git reset mod/* && git clean -f mod/*");
...
Процедура ПродолжениеПослеСлияния() Экспорт
	...
	ФайлСоСпискомОбъектов = "temp/СписокОбъектовХранилиза1С.xml";
